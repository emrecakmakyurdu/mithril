...
from mithril.models import Linear, Mean, Model, Relu, SquaredError, TrainModel, Transpose
...
def test_primitives_c_2():
    ...
    for i in range(100):
        outputs, grads = np_pm.evaluate(params, data=inputs, output_gradients=True)
        c_outputs, c_grads = c_pm.evaluate(c_params, data=c_inputs, output_gradients=True)
        ggml_outputs, ggml_grads = ggml_pm.evaluate(ggml_params, data=ggml_imputs, output_gradients=True)
            
        for key, grad in c_grads.items():
            c_params[key] = c_params[key] - 0.01 * grad
        
        for key, grad in ggml_grads.items():
            ggml_params[key] = ggml_params[key] - 0.01 * grad

        for key, grad in grads.items():
            params[key] = params[key] - 0.01 * grad
            assert np.allclose(cbackend.to_numpy(c_grads[key]), grads[key], rtol=1e-2, atol=1e-2)
            assert np.allclose(cbackend.to_numpy(c_params[key]), params[key], rtol=1e-2, atol=1e-2)
            
            assert np.allclose(ggml_backend.to_numpy(ggml_grads[key]), grads[key], rtol=1e-2, atol=1e-2)
            assert np.allclose(ggml_backend.to_numpy(ggml_params[key]),params[key], rtol=1e-2, atol=1e-2)
...
def test_primitives_c_3():
    ...
    right = backend.ones([8,3])
    target = backend.ones([4,3])
    ...
    ggml_outputs = ggml_pm.evaluate(ggml_params,data=ggml_imputs, output_gradients=False)
    assert np.allclose(outputs["output"], ggml_backend.to_numpy(ggml_outputs["output"]))